...
export default function OrderDetailNotes() {
  const [orderDetailData, setOrderDetailData] = useState([
    ...
    {
      icon: <FaHashtag className="text-blue-600" />,
      title: "Order Number",
      value: "Teamspring",
    },
    {
    ...
    <div className="p-3 bg-white rounded-lg shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {orderDetailData.map((item, index) => (
          <div
            key={index}
            className="flex items-start bg-[#F8FAFB] p-4 rounded-lg"
          >
            <div className="text-2xl mr-3">{item.icon}</div>
            <div>
              <p className="text-sm text-[#969BA0]">{item.title}</p>
              <p className="font-semibold text-[#202020]">{item.value}</p>
            </div>
          </div>
        ))}
      </div>
